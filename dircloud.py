...
def dircloud(dirpath='/'):
    ...
    if directory:
        ...
        header = '%s directories, %s' % (entries, human_readable(total_size))
        footer = '%s directories from %s' % (len(du), settings['filename'])
    else:
...
def make_html_page(dirpath='', header='', search='', body='', footer=''):
    ...
    crumbs.append(' <span class="filesize"><a href="%(href)s">(%(filesize)s)</a></span>' %
                  {'href': read_from_disk,
...
''' % ({'search': search,
    ...
    footer += '\n <div class="stale_info">Web interface and CSS inspired and borrowed from <a href="http://repo.or.cz/">http://repo.or.cz/</a></div>'
    if settings['search_client'] == 'dicoclient':
        footer += '\n <div class="stale_info">Search powered by <a href="http://www.dict.org/">http://www.dict.org/</a></div>'
    footer += '\n</body>\n'
    footer += '\n</html>\n'
